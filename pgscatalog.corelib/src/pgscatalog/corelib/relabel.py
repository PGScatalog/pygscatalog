...
def relabel_write(relabelled, dataset, split_output, combined_output, out_dir):
    ...
    >>> import tempfile, os
    >>> maps = [Config.ROOT_DIR / "tests" / "relabel_map_chr1.txt.gz", Config.ROOT_DIR / "tests" / "relabel_map_chr8.txt.gz"]
    >>> in_target = Config.ROOT_DIR / "tests" / "hgdp_ALL_additive_0.scorefile"
    >>> args = RelabelArgs()
